CREATE TABLE wwv_mig_olb_visualattribute (
  ...
  CONSTRAINT wwv_mig_olb_visualattribute_pk PRIMARY KEY ("ID") USING INDEX NOCOMPRESS
    PCTFREE 10 INITRANS 2 LOGGING
    STORAGE(FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    TABLESPACE "SYSAUX",
  CONSTRAINT wwv_mig_olb_va_objlib_id_fk FOREIGN KEY (objectlibrary_id) REFERENCES wwv_mig_olb_objectlibrary ("ID") ON DELETE CASCADE,
  CONSTRAINT wwv_mig_olb_va_sg_id_fk FOREIGN KEY (security_group_id) REFERENCES wwv_flow_companies (provisioning_company_id) ON DELETE CASCADE
) NOCOMPRESS
