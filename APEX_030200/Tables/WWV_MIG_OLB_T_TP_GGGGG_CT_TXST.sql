CREATE TABLE wwv_mig_olb_t_tp_ggggg_ct_txst (
  ...
  CONSTRAINT wwv_mig_olb_ttpgggggct_txst_pk PRIMARY KEY ("ID") USING INDEX NOCOMPRESS
    PCTFREE 10 INITRANS 2 LOGGING
    STORAGE(FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    TABLESPACE "SYSAUX",
  CONSTRAINT wwv_mig_olb_ttp5gcts_ct_id_fk FOREIGN KEY (compoundtext_id) REFERENCES wwv_mig_olb_t_tp_ggggg_cpdtxt ("ID") ON DELETE CASCADE,
  CONSTRAINT wwv_mig_olb_ttp5gcts_sg_id_fk FOREIGN KEY (security_group_id) REFERENCES wwv_flow_companies (provisioning_company_id) ON DELETE CASCADE
) NOCOMPRESS
