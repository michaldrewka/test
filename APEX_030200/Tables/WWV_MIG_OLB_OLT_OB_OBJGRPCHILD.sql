CREATE TABLE wwv_mig_olb_olt_ob_objgrpchild (
  ...
  CONSTRAINT wwv_mig_olb_olt_ob_ogchild_pk PRIMARY KEY ("ID") USING INDEX NOCOMPRESS
    PCTFREE 10 INITRANS 2 LOGGING
    STORAGE(FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    TABLESPACE "SYSAUX",
  CONSTRAINT wwv_mig_olb_olt_ob_ogc_obid_fk FOREIGN KEY (objectgroup_id) REFERENCES wwv_mig_olb_olt_objectgroup ("ID") ON DELETE CASCADE,
  CONSTRAINT wwv_mig_olb_olt_ob_ogc_sgid_fk FOREIGN KEY (security_group_id) REFERENCES wwv_flow_companies (provisioning_company_id) ON DELETE CASCADE
) NOCOMPRESS
