CREATE TABLE wwv_mig_olb_blk_item_trigger (
  ...
  select_for_app VARCHAR2(1 BYTE) DEFAULT 'N' NOT NULL CONSTRAINT wwv_mig_olb_bit_sel_for_app CHECK (select_for_app in ('Y', 'N')),
  ...
  CONSTRAINT wwv_mig_olb_blk_itm_trg_pk PRIMARY KEY ("ID") USING INDEX NOCOMPRESS
    PCTFREE 10 INITRANS 2 LOGGING
    STORAGE(FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    TABLESPACE "SYSAUX",
  CONSTRAINT wwv_mig_olb_bit_item_id_fk FOREIGN KEY (item_id) REFERENCES wwv_mig_olb_blk_item ("ID") ON DELETE CASCADE,
  CONSTRAINT wwv_mig_olb_bit_sg_id_fk FOREIGN KEY (security_group_id) REFERENCES wwv_flow_companies (provisioning_company_id) ON DELETE CASCADE
) NOCOMPRESS
