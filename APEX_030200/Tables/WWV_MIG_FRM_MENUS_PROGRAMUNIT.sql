CREATE TABLE wwv_mig_frm_menus_programunit (
  ...
  CONSTRAINT wwv_mig_mnuprogunit_pk PRIMARY KEY ("ID") USING INDEX NOCOMPRESS
    PCTFREE 10 INITRANS 2 LOGGING
    STORAGE(FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT)
    TABLESPACE "SYSAUX",
  CONSTRAINT wwv_mig_mnuprogunit_sg_id_fk FOREIGN KEY (security_group_id) REFERENCES wwv_flow_companies (provisioning_company_id) ON DELETE CASCADE,
  CONSTRAINT wwv_mig_mnu_progunit_id_fk FOREIGN KEY (menumodule_id) REFERENCES wwv_mig_frm_menus_menumodules ("ID") ON DELETE CASCADE
) NOCOMPRESS
